...
class HermesCLI:
    ...
    def run(self):
        ...
        # Do not register Ctrl+Shift+C here: prompt-toolkit does not accept
        # the string form ``c-S-c`` (it raises ``Invalid key: c-S-c`` during
        # CLI startup). Terminal emulators handle Ctrl+Shift+C/Cmd+C copy
        # natively before the application sees it, so Hermes does not need an
        # application-level no-op binding.
